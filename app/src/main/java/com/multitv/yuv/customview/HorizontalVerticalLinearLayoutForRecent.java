...
public class HorizontalVerticalLinearLayoutForRecent extends ViewGroup {
    private static final String TAG = AppConfig.BASE_TAG + ".HorizontalVerticalLinearLayoutForRecent";
    boolean showAll = false;
    int rowCounter;
    ImageView ivMore;

    public HorizontalVerticalLinearLayoutForRecent(Context context) {
        super(context);
    }

    public HorizontalVerticalLinearLayoutForRecent(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public HorizontalVerticalLinearLayoutForRecent(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public HorizontalVerticalLinearLayoutForRecent(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (getChildCount() > 0) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (left + measuredWidth <= maxWidth) {
                    ...
                    //Tracer.info(TAG, "onLayout() : for child : " + i);
                } else {
                    counter += 1;
                    rowCounter = counter;
                    if (showAll) {
                        ...
                        if (rowCount < 2) {
                            ...
                            left = measuredWidth + padding;
                            maxHeightInRow = 0;
                            //Tracer.info(TAG, "onLayout() : for child when row less than 2: " + i);
                            rowCount = rowCount + 1;
                        } else {
                            Tracer.info(TAG, "onLayout() : for child when row greater than 2: " + i);
                            break;
                        }
                    }
                }
        ...
        if (getChildCount() > 0) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (caluclatedWidth + measuredWidth <= maxWidth) {
                    ...
                    //Tracer.info(TAG, "onMeasure() : for child : " + i);
                } else {
                    if (showAll) {
                        ...
                        if (rowCount < 2) {
                            ...
                            maxHeightInRow = 0;
                            rowCount = rowCount + 1;
                            //Tracer.info(TAG, "onMeasure() when row count less than 2: for child : " + i);
                        } else {
                            //Tracer.info(TAG, "onMeasure() when row count greater than 2: for child : " + i);
                            break;
                        }
                    }
                }
