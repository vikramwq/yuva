...
public class VideoPlaylistAdapter extends RecyclerView.Adapter {
    ...
    public VideoPlaylistAdapter(Context context, List<Cat_cntn> recomendedArrayList
    ) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_ITEM) {
            ...
            return new VideoPlaylistAdapter.SingleItemRowHolder(v);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VideoPlaylistAdapter.SingleItemRowHolder) {
            final Cat_cntn content = itemsList.get(position);
            if (content != null) {
                if (content.title != null && !content.title.isEmpty())
                    ((VideoPlaylistAdapter.SingleItemRowHolder) holder).mTitle.setText(content.title);
                else
                    ((VideoPlaylistAdapter.SingleItemRowHolder) holder).mTitle.setVisibility(View.GONE);

                String description = content.des;
                if (description != null && !TextUtils.isEmpty(description)) {
                    ((VideoPlaylistAdapter.SingleItemRowHolder) holder).source_type.setText(description.trim());
                } else
                    ((VideoPlaylistAdapter.SingleItemRowHolder) holder).source_type.setVisibility(View.GONE);
...
//                    ((SingleItemRowHolder) holder).genre_sony.setVisibility(View.GONE);
                ...
                if (duration != null && !duration.equals(null) && !TextUtils.isEmpty(duration)) {
                    ...
                    if (hours.equals("00")) {
                        ((VideoPlaylistAdapter.SingleItemRowHolder) holder).duration.setText(minutes + ":" + seconds + " " + mContext.getResources().getString(R.string.minute));
                    } else {
                        ((VideoPlaylistAdapter.SingleItemRowHolder) holder).duration.setText(hours + ":" + minutes + " " + mContext.getResources().getString(R.string.hours));
                    }
                } else {
                    ((VideoPlaylistAdapter.SingleItemRowHolder) holder).duration.setVisibility(View.GONE);
                }
                String url = null;
                ...
                if (url != null && !url.isEmpty()) {
                    Picasso
                            ...
                            .resize(((VideoPlaylistAdapter.SingleItemRowHolder) holder).mImageView.getWidth(), (int) mContext.getResources().getDimension(R.dimen._100sdp))
                            .into(((VideoPlaylistAdapter.SingleItemRowHolder) holder).mImageView);
